        ...
        <div class="glass-card rounded-[18px] p-8 shadow-medium transition duration-300 animate-slide-up">
          ...
          @if (mode === 'login') {
            ...
            <div class="mt-8">
              ...
              <div class="mt-6 flex justify-center w-full min-h-[44px] relative">
                <!-- Our custom button rendered in the background -->
                <button type="button" (click)="onGoogleSignIn()" [disabled]="isGoogleLoading() || isLoading()"
                ...
                <!-- Transparent Google Sign-In SDK button overlaid exactly on top -->
                <div id="googleBtnContainer"
                     [class.hidden]="!isGoogleSdkLoaded() || isGoogleLoading() || isLoading()"
                     class="absolute inset-0 w-full h-full opacity-[0.01] overflow-hidden z-20"></div>
              </div>
            </div>
          }
...
export class LoginComponent implements OnInit {
  ...
  isGoogleSdkLoaded = signal<boolean>(false);
  ...
  ngOnInit(): void {
    ...
    this.authService.getGoogleClientId().subscribe({
      next: (res) => {
        this.googleClientId = res.data || '';
        if (this.googleClientId) {
          this.initializeGoogleSignIn(this.googleClientId);
        } else {
          this.isGoogleSdkLoaded.set(false);
        }
      },
      error: () => {
        this.isGoogleSdkLoaded.set(false);
  ...
  private initializeGoogleSignIn(clientId: string) {
    ...
    try {
          ...
          if (response.credential) {
            ...
            this.authService.googleLogin(response.credential).subscribe({
              ...
              error: (err) => {
                ...
                if (err.status === 0) {
                  errorMsg = 'Backend server is unavailable. Please check if the service is running.';
                } else if (err.status === 401 || err.status === 403) {
                  errorMsg = 'Invalid or expired Google token.';
      ...
      // Render the visible native Google button overlay on the container
  ...
  onGoogleSignIn() {
    // If the Google SDK has loaded, this fallback is never triggered as the transparent button handles it.
    // If it triggers, the SDK has not loaded.
    this.toastr.error('Google Sign-In SDK is not loaded. Please check your network or adblocker settings.', 'Error');
  }
}
