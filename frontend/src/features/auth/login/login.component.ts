      ...
      <div class="w-full max-w-md space-y-8 z-10">
        ...
        <div class="flex flex-col items-center">
          ...
          <p class="mt-2 text-sm text-slate-400">
            @if (mode === 'login') {
              Or
              <a routerLink="/register" class="font-semibold text-indigo-400 hover:text-indigo-300 transition duration-150">
                create a new account
              </a>
            } @else {
              <button (click)="switchMode('login')" class="font-semibold text-indigo-400 hover:text-indigo-300 transition duration-150 bg-transparent border-0 p-0 cursor-pointer">
                Return to sign in
              </button>
            }
          </p>
        ...
        <div class="bg-slate-900/40 border border-slate-800/80 backdrop-blur-xl rounded-3xl p-8 shadow-2xl shadow-black/40 hover:border-slate-800 transition duration-300">
          ...
          @if (mode === 'login') {
            <form [formGroup]="loginForm" (ngSubmit)="onLoginSubmit()" class="space-y-6">
              <div>
                ...
                <div class="mt-1.5">
                  <input id="email" type="email" formControlName="email" required
                         class="w-full bg-slate-950/60 border border-slate-800 rounded-xl px-4 py-3 text-slate-200 placeholder-slate-600 focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition duration-200"
                         placeholder="developer@devmind.ai">
                  @if (loginForm.get('email')?.touched && loginForm.get('email')?.invalid) {
                    <p class="mt-1.5 text-xs text-rose-500 font-medium">Please enter a valid email address.</p>
                  }
                </div>
              </div>
              ...
              <div>
                <div class="flex items-center justify-between">
                  ...
                  <button type="button" (click)="switchMode('request-reset')" class="text-xs font-semibold text-indigo-400 hover:text-indigo-300 transition duration-150">
                    Forgot password?
                  </button>
                </div>
                <div class="mt-1.5">
                  ...
                  @if (loginForm.get('password')?.touched && loginForm.get('password')?.invalid) {
                    <p class="mt-1.5 text-xs text-rose-500 font-medium">Password is required.</p>
                  }
                </div>
              </div>
            ...
            <div class="mt-8">
              <div class="relative">
                ...
                <div class="relative flex justify-center text-xs uppercase"><span class="bg-slate-900 px-3 text-slate-500 font-semibold tracking-wider">Or continue with</span></div>
              </div>

              <div class="mt-6">
                <button (click)="onGoogleSignIn()"
                        class="w-full flex items-center justify-center gap-3 px-4 py-3 border border-slate-800 hover:border-slate-700 bg-slate-950/40 rounded-xl text-sm font-semibold text-slate-300 hover:text-white transition duration-200 shadow shadow-black/10">
          ...
          @if (mode === 'request-reset') {
            <form [formGroup]="requestResetForm" (ngSubmit)="onRequestResetSubmit()" class="space-y-6">
              ...
              <div>
                <label for="reset-email" class="block text-xs font-semibold uppercase tracking-wider text-slate-400">Email Address</label>
                <div class="mt-1.5">
          ...
          @if (mode === 'confirm-reset') {
            <form [formGroup]="confirmResetForm" (ngSubmit)="onConfirmResetSubmit()" class="space-y-6">
              <div>
                <label for="reset-token" class="block text-xs font-semibold uppercase tracking-wider text-slate-400">Reset Token</label>
                <div class="mt-1.5">
                  ...
                  @if (confirmResetForm.get('token')?.touched && confirmResetForm.get('token')?.invalid) {
                    <p class="mt-1.5 text-xs text-rose-500 font-medium">Reset token is required.</p>
                  }
                </div>
              </div>

              <div>
                <label for="new-password" class="block text-xs font-semibold uppercase tracking-wider text-slate-400">New Password</label>
                <div class="mt-1.5">
                  ...
                  @if (confirmResetForm.get('newPassword')?.touched && confirmResetForm.get('newPassword')?.invalid) {
                    <p class="mt-1.5 text-xs text-rose-500 font-medium">Password must meet all complexity requirements.</p>
                  }
                </div>
              </div>
